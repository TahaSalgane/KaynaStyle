...
<head>
    ...
    <style>
        ...
        .text-brown-lighter {
            color: var(--color-text-brown-lighter);
        }
        ...
        .ring-brown {
            --tw-ring-color: var(--color-brown-primary);
        }
        ...
        .hover\:bg-brown-hover:hover {
            background-color: var(--color-brown-hover);
        }

        .hover\:bg-brown-light:hover {
            background-color: var(--color-brown-hover-light);
        }
...
<body class="bg-gray-50 {{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    @if (isset($activeCountdown) && $activeCountdown)
        ...
        <div id="countdown-banner" class="countdown-banner"
            style="background: linear-gradient(135deg, {{ $activeCountdown->background_color }}, {{ $activeCountdown->background_color }}dd); color: {{ $activeCountdown->text_color }};">
                ...
                <div class="flex items-center justify-center flex-wrap gap-3 sm:gap-4 text-center">
                    @if ($activeCountdown->title)
                        <span class="font-bold text-sm sm:text-base animate-pulse countdown-title">
                            <i
                                class="fas fa-fire {{ app()->getLocale() === 'ar' ? 'ml-1 sm:ml-2' : 'mr-1 sm:mr-2' }}"></i>{{ $activeCountdown->title }}
                        </span>
                    @endif
            ...
            <span class="countdown-end-date hidden"
    ...
    <nav class="bg-white/95 backdrop-blur-md shadow-xl sticky top-0 z-50 border-b border-brown/10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 sm:h-18">
                ...
                <div class="hidden lg:block">
                    <div
                        class="{{ app()->getLocale() === 'ar' ? 'mr-10' : 'ml-10' }} flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-1' : 'space-x-1' }}">
                        <a href="{{ route('home') }}"
                            class="nav-link text-gray-700 hover:text-brown hover:bg-brown/5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300">
                            <i
                                class="fas fa-home {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('messages.home') }}
                        </a>
                        <a href="{{ route('products.index') }}"
                            class="nav-link text-gray-700 hover:text-brown hover:bg-brown/5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300">
                            <i
                                class="fas fa-shopping-bag {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('messages.products') }}
                        </a>
                        <a href="{{ route('categories.index') }}"
                            class="nav-link text-gray-700 hover:text-brown hover:bg-brown/5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300">
                            <i
                                class="fas fa-th-large {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('messages.categories') }}
                        </a>
                        <a href="{{ route('faq.index') }}"
                            class="nav-link text-gray-700 hover:text-brown hover:bg-brown/5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300">
                            <i
                                class="fas fa-question-circle {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('messages.faq') }}
                        </a>
                        <a href="{{ route('contact.index') }}"
                            class="nav-link text-gray-700 hover:text-brown hover:bg-brown/5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300">
                            <i
                                class="fas fa-envelope {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('messages.contact_us') }}
                        </a>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="relative group">
                        <button onclick="toggleLanguageMenu()"
                            class="flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-2' : 'space-x-2' }} bg-white border border-gray-200 hover:border-brown rounded-lg px-3 py-2 text-sm font-medium transition-all duration-300 hover:shadow-md">
                            <span id="current-flag" class="w-6 h-4 rounded-sm overflow-hidden">
                                @if (app()->getLocale() === 'ar')
                                    <img src="https://flagcdn.com/w20/ma.png" alt="Morocco Flag"
                                        class="w-full h-full object-cover">
                                @elseif(app()->getLocale() === 'en')
                                    <img src="https://flagcdn.com/w20/gb.png" alt="UK Flag"
                                        class="w-full h-full object-cover">
                                @elseif(app()->getLocale() === 'fr')
                                    <img src="https://flagcdn.com/w20/fr.png" alt="French Flag"
                                        class="w-full h-full object-cover">
                                @else
                        ...
                        <div id="language-menu"
                            class="hidden absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} mt-2 w-28 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
                            @if (app()->getLocale() !== 'ar')
                                <a href="{{ route('lang.switch', 'ar') }}"
                                    class="flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-3' : 'space-x-3' }} px-4 py-2 text-sm text-gray-700 hover:bg-brown/5 hover:text-brown transition-colors duration-200">
                                    <img src="https://flagcdn.com/w20/ma.png" alt="Morocco Flag"
                    ...
                    <button onclick="openCartSidebar()" type="button" class="relative group">
                        <div
                            class="p-2 rounded-lg hover:bg-brown/5 transition-all duration-300 group-hover:scale-110 relative">
                            <i
                                class="fas fa-shopping-cart text-lg sm:text-xl text-gray-700 group-hover:text-brown transition-colors duration-300"></i>
                            <span id="cart-count"
                                class="absolute -top-1 -right-1 bg-gradient-to-r from-red-500 to-red-600 text-white text-[10px] sm:text-xs rounded-full min-w-[18px] sm:min-w-[20px] h-[18px] sm:h-[20px] flex items-center justify-center font-bold shadow-lg border-2 border-white">
                                0
                            </span>
                        </div>
                    </button>
                    ...
                    <div class="lg:hidden">
                        <button type="button" onclick="toggleMobileMenu()"
                            class="p-2 rounded-lg text-gray-700 hover:text-brown hover:bg-brown/5 focus:outline-none focus:text-brown transition-all duration-300">
        ...
        <div class="lg:hidden hidden" id="mobile-menu">
            <div class="px-4 pt-2 pb-4 space-y-1 bg-white/95 backdrop-blur-md border-t border-brown/10">
                <a href="{{ route('home') }}"
                    class="flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-3' : 'space-x-3' }} text-gray-700 hover:text-brown hover:bg-brown/5 block px-4 py-3 rounded-lg text-base font-medium transition-all duration-300">
    ...
    <!-- Newsletter Modal -->
    <div id="newsletterModal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4"
        style="background-color: rgba(0, 0, 0, 0.5); backdrop-filter: blur(4px);">
        <div class="relative bg-white rounded-3xl shadow-2xl max-w-2xl w-full transform transition-all duration-300 scale-95 opacity-0 overflow-hidden"
            id="newsletterModalContent">
            <!-- Close Button -->
            <button onclick="closeNewsletterModal()"
                class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors duration-200 z-10 bg-white/80 rounded-full p-2 hover:bg-white">
            ...
            <div class="flex flex-col md:flex-row">
                <!-- Left Side - Image -->
                <div
                    class="hidden md:block md:w-3/5 bg-gradient-to-br from-brown to-brown-darker flex items-center justify-center">
                    <img src="{{ asset('images/newsletter-modal.webp') }}" alt="Newsletter"
                ...
                <div class="w-full md:w-3/6 p-6 sm:p-8 bg-white">
                    ...
                    <form id="newsletterModalForm" class="space-y-4">
                        <div>
                            <input type="email" id="newsletterModalEmail" name="email"
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        <label class="flex items-center justify-center cursor-pointer">
                            <input type="checkbox" id="dontShowAgain"
                                class="{{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} w-4 h-4 text-brown focus:ring-brown border-gray-300 rounded">
    ...
    <div id="liveSaleNotification"
        class="fixed bottom-4 left-4 md:bottom-6 md:left-6 z-50 hidden transform transition-all duration-500 ease-out"
        style="transform: translateX(-120%); opacity: 0;">
        <div class="bg-white rounded-xl shadow-2xl border border-gray-200 p-4 md:p-5 max-w-sm md:max-w-sm w-full flex items-center gap-4 md:gap-4 relative"
            style="box-shadow: 0 10px 25px rgba(0, 0, 0, 0.15);">
            <!-- Close Button -->
            <button onclick="closeLiveSaleNotification()"
                class="absolute top-2.5 right-2.5 md:top-3 md:right-3 text-gray-400 hover:text-gray-600 transition-colors z-10 bg-white/80 rounded-full p-1.5 md:p-1.5">
                <i class="fas fa-times text-sm md:text-sm"></i>
            </button>

            <!-- Product Image -->
            <div class="flex-shrink-0">
                <img id="notificationProductImage" src="" alt="Product"
                    class="w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-lg object-cover border border-gray-200 shadow-md">
            </div>
    ...
    <div id="cart-sidebar-overlay"
        ...
        onclick="closeCartSidebar()"></div>
    <div id="cart-sidebar"
        class="fixed top-0 bottom-0 w-3/4 sm:w-full max-w-md bg-white shadow-2xl z-[60] transform transition-transform duration-300 ease-in-out overflow-hidden right-0 translate-x-full">
        <div class="flex flex-col h-full">
            <!-- Header -->
            <div class="flex items-center justify-between p-4 sm:p-6 border-b border-brown/20"
                style="background: linear-gradient(135deg, var(--color-brown-primary), var(--color-brown-darker));">
                <h2
                    class="text-xl sm:text-2xl font-bold text-white flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-2' : 'space-x-2' }}">
                    <i class="fas fa-shopping-cart text-white"></i>
                    <span class="text-white">{{ __('messages.shopping_cart') }}</span>
                    <span id="cart-sidebar-count"
                        class="text-sm bg-white/30 text-white px-2 py-1 rounded-full font-semibold">(0)</span>
                </h2>
                <button onclick="closeCartSidebar()"
                    class="text-white hover:text-white/80 transition-colors duration-200 p-2 rounded-lg hover:bg-white/20">
            ...
            <div id="cart-sidebar-footer" class="hidden border-t border-gray-200 bg-gray-50 p-4 sm:p-6">
                ...
                <a href="{{ route('checkout') }}"
                    class="block w-full btn-brown-gradient text-center py-3 rounded-lg font-semibold text-white transition-all duration-300 transform hover:scale-105">
                    <i class="fas fa-credit-card {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>
                    {{ __('messages.checkout') }}
                </a>
                <a href="{{ route('cart.index') }}"
                    class="block w-full text-center py-2 mt-2 text-brown hover:text-brown-darker transition-colors duration-200 text-sm font-medium">
    ...
    <footer class="text-white py-6"
        style="background: linear-gradient(to right, var(--color-brown-primary), var(--color-brown-darker));">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                ...
                <div>
                    ...
                    <div
                        class="flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-4' : 'space-x-4' }}">
                    ...
                    <ul class="space-y-1 sm:space-y-2">
                        @foreach ($footerCategories ?? [] as $category)
                            <li><a href="{{ route('categories.show', $category->slug) }}"
                                    class="text-gray-300 hover:text-white text-sm sm:text-base transition-colors">{{ $category->name_en }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-1 sm:space-y-2">
                        <li><a href="{{ route('policies.privacy') }}"
                                class="text-gray-300 hover:text-white text-sm sm:text-base transition-colors">{{ __('messages.privacy_policy') }}</a>
                        </li>
                        <li><a href="{{ route('policies.shipping') }}"
                                class="text-gray-300 hover:text-white text-sm sm:text-base transition-colors">{{ __('messages.shipping_policy') }}</a>
                        </li>
                        <li><a href="{{ route('contact.index') }}"
                                class="text-gray-300 hover:text-white text-sm sm:text-base transition-colors">{{ __('messages.contact_us') }}</a>
                        </li>
                        <li><a href="{{ route('wishlist.index') }}"
                                class="text-gray-300 hover:text-white text-sm sm:text-base transition-colors">{{ __('messages.wishlist') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <script>
            ...
            fetch('{{ route('cart.sidebar') }}', {
                ...
                .then(data => {
                    ...
                    if (data.success && data.items && data.items.length > 0) {
                        ...
                        // Update total
                        document.getElementById('cart-sidebar-total').textContent =
                            `${parseFloat(data.total).toFixed(2)} DH`;
        ...
        if (newsletterModalForm) {
            newsletterModalForm.addEventListener('submit', function(e) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin ' + (@json(app()->getLocale() === 'ar') ? 'ml-2' :
                    'mr-2') + '"></i>' + submittingText + '...';
